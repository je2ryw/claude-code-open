...
export class VisualizationServer {
  ...
  private handleApiRequest(
    ...
    console.log('[DEBUG handleApiRequest] pathname:', pathname);
    try {
      ...
      console.log('[DEBUG] Checking smart-hover, pathname:', pathname);
      if (pathname === '/api/smart-hover') {
        console.log('[DEBUG] Matched smart-hover API');
        const moduleId = url.searchParams.get('module');
        const startLine = parseInt(url.searchParams.get('start') || '1', 10);
        const endLine = parseInt(url.searchParams.get('end') || '10', 10);
...
<body>
  ...
  <script>
    ...
    async function showSmartHover(selection) {
      if (!monacoEditor || !currentCodeModuleId) return;
      ...
      try {
        const response = await fetch(
          '/api/smart-hover?module=' + encodeURIComponent(currentCodeModuleId) +
      ...
      if (data.analysis?.localRole) {
        html += \`
          <div class="smart-hover-section local-role">
            ...
            <div class="smart-hover-section-content">\${data.analysis.localRole}</div>
          </div>
        \`;
      }
      ...
      if (data.analysis?.globalRole) {
        html += \`
          <div class="smart-hover-section global-role">
            ...
            <div class="smart-hover-section-content">\${data.analysis.globalRole}</div>
          </div>
        \`;
      }
      ...
      if (data.analysis?.workingPrinciple) {
        html += \`
          <div class="smart-hover-section principle">
            ...
            <div class="smart-hover-section-content">\${data.analysis.workingPrinciple}</div>
          </div>
        \`;
      }
          ...
          html += \`
            <div class="dependency-item">
              <div>
                ...
                <div class="desc">\${dep.description}</div>
              </div>
            </div>
          \`;
        });
